...
# Q3: OCTOBER 2023 ESTIMATION SECTION
    ...
    for date in all_oct_dates:
        if date in oct_2023['posting_date'].values:
            volume = oct_2023[oct_2023['posting_date'] == date]['volume_gbp'].values[0]
    ...
    fig = go.Figure(data=go.Heatmap(
        z=calendar_df['Day'].values.reshape(-1, 7).T,
        x=['Week 1', 'Week 2', 'Week 3', 'Week 4', 'Week 5'][:calendar_df['Day'].values.reshape(-1, 7).shape[1]],
        y=['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
        text=[[f"Day {d}<br>{'Available' if d in oct_2023['posting_date'].dt.day.values else 'Missing'}" 
               for d in week] for week in calendar_df['Day'].values.reshape(-1, 7).T],
        texttemplate="%{text}",
        textfont={"size": 10},
    ...
    fig.update_layout(
        title='October 2023 Data Availability Calendar',
        height=300,
        template='plotly_white'
    )
